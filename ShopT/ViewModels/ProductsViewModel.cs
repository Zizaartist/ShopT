    ...
    public class ProductsViewModel : CollectionViewModel
    {
        ...
        public async Task GetRemoteData()
        {
            ...
            try
            {
                ...
                HttpResponseMessage response = await client.GetAsync(ApiStrings.HOST + ApiStrings.PRODUCTS_GET_BY_MENU + menu.CategoryId + "/" + NextPage);
                if (response.IsSuccessStatusCode)
                {
                    ...
                    await BlobCache.LocalMachine.InsertObject($"{ShopInfoStatis.shopInfo.ShopId}_{Caches.PRODUCTS_CACHE.key}" + "_" + menu.CategoryId.ToString(), (NextPage, ProductLists.Select(e => e.Product)), Caches.PRODUCTS_CACHE.lifeTime);
                }
        ...
        public async Task GetCachedData()
        {
            ...
            (int, List<Product>) cachedProducts = await new CacheFunctions().tryToGet<(int, List<Product>)>($"{ShopInfoStatis.shopInfo.ShopId}_{Caches.PRODUCTS_CACHE.key}" + "_" + menu.CategoryId.ToString(), CacheFunctions.BlobCaches.LocalMachine);
            ...
            if (cachedProducts != default)
            {
                ...
                {
                    localizedList.Add(new ProductLocal(product, UpdateBindings, AddToSelected, RemoveFromSelected, AddToBasket));
                }
